...
with st.sidebar:
    ...
    crypto_dict = {
        ...
        "Chainlink": "LINK-USD", "Tether": "USDT-USD", "Shiba Inu": "SHIB-USD"
    }
    pilihan = st.multiselect(
        "Pilih crypto untuk grafik dan prediksi:", list(crypto_dict.keys()), default=["Tether", "Ethereum"]
    )
...
st.subheader("🔮 Prediksi Harga Cryptocurrency")
...
for t in tickers:
    ...
    try:
        # Uji stasioneritas 
        model = ARIMA(df_ts['Close'], order=(3,1,2))  
        model_fit = model.fit()
...
if prediksi_df_raw.empty:
    ...
    for date_idx, date in enumerate(prediksi_df_raw.index):
        row_data = {"Tanggal": date.strftime('%Y-%m-%d')}
